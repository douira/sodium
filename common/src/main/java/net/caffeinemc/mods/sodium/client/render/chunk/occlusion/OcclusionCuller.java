...
public class OcclusionCuller {
        ...
        while ((section = readQueue.dequeue()) != null) {
...

            {
                if (this.useOcclusionCulling) {
                    ...
                    connections = VisibilityEncoding.getConnections(sectionVisibilityData, this.getSectionIncomingDirections(section));
                } else {
                ...
                // directions.
                connections &= getOutwardDirections(this.viewport.getChunkCoord(), section);
            }
    ...
    boolean sectionIsUnvisited(RenderSection section) {
        return section.getLastVisibleFrame() != this.frame;
    }

    void setSectionVisitedInitial(RenderSection section) {
        section.setLastVisibleFrame(this.frame);
    ...
    int getSectionIncomingDirections(RenderSection section) {
        return section.getIncomingDirections();
    }

    private void visitNode(WriteQueue<RenderSection> queue, RenderSection render, int incoming) {
        // isn't usually null, but can be null if the bfs is happening during loading or unloading of chunks
        if (render == null) {
            return;
        }

        if (this.sectionIsUnvisited(render)) {
            // This is the first time we are visiting this section during the given token, so we must
            // reset the state.
            this.setSectionVisitedInitial(render);

            if (isWithinRenderDistance(this.viewport.getTransform(), render, this.searchDistance) && this.isWithinFrustum(this.viewport, render)) {
                this.visitor.visit(render);
                queue.enqueue(render);
            }
        }

        this.addSectionIncomingDirections(render, incoming);
    }
        ...
        for (var dx = -1; dx <= 1; dx++) {
            for (var dy = -1; dy <= 1; dy++) {
                for (var dz = -1; dz <= 1; dz++) {
                    ...
                    if (section != null && this.sectionIsUnvisited(section) && isWithinNearbySectionFrustum(viewport, section)) {
                        // reset state on first visit, but don't enqueue
                        this.setSectionVisitedInitial(section);
